...
class AdvancedRRTStarPathPlanner:
    def __init__(self, initial_x, initial_y, final_x, final_y, traversability_weight, 
        ...
        self.goal_radius = 3.0 * self.scale # Radius for goal proximity
        self.min_step_size = 3.0 * self.scale # Minimum step size for each iteration
        self.step_size = 4.0 * self.scale # Maximum step size for each iteration
        self.search_radius = 3.0 * self.scale # Search radius for nearby nodes
    ...
    def steer_travs(self, from_node, calculated_node):
        ...
        goal_direction = np.array(calculated_node) - np.array(from_node)
        goal_direction = goal_direction / np.linalg.norm(goal_direction)
        
        best_node = calculated_node
        best_score = -float('inf')
        
        for x in range(frontier.shape[1]):
            for y in range(frontier.shape[0]):
                new_node = (minX + x, minY + y)
                if not self.is_within_boundary(new_node):
                    continue
                
                # Calculate direction to new node
                node_direction = np.array(new_node) - np.array(from_node)
                if np.linalg.norm(node_direction) > 0:
                    node_direction = node_direction / np.linalg.norm(node_direction)
                
                # Calculate dot product to favor directions towards the goal
                direction_score = np.dot(goal_direction, node_direction)
                
                # Combine traversability and direction scores
                traversability_score = frontier[y, x]
                combined_score = 0.9 * traversability_score + 0.1 * direction_score
                        ...
                        if self.distance(new_node, self.goal_node) < self.goal_radius:
                            ...
                            if goal_traversability > 0:  # Ensure the goal node is in a traversable area
                                ...
                                for near_node in self.near_nodes(self.goal_node):
                                    ...
                                    traversability_near = self.get_traversability(near_node)
                                    if traversability_near == 1:  # Skip nodes that are in impassable areas
                                        continue
